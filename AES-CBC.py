...
def hex_to_dec(hex_num, flag):
    # 十六进制矩阵转换为十进制矩阵
    number = int(hex_num, 16)
    int_num = number // 16
    int_re = number % 16
    if flag == 1:
        my_number = SBox(int_num, int_re)
    else:
        my_number = InvSBox(int_num, int_re)
    return my_number


def SubBytes(dir_new_number, flag):
    # 定义字节代换
    dir_1 = {0: [], 1: [], 2: [], 3: []}
    for j in range(4):
        list_new = []
        ...
        for k in range(4):
            new_num = hex_to_dec(list[k], flag)
            list_new.append(new_num)
        dir_1[j] = list_new
    return dir_1


def ShiftRows(dir_clear_number):
    # 进行行移位操作
    for i in range(4):
        my_list = []
...
def InvShiftRows(dir_clear_number):
    # 进行行移位的逆操作
    for i in range(4):
        my_list = []
        list = dir_clear_number[i]
...
def MixColumns(dir_clear_number_copy):
...
def InvMixColumns(dir_clear_number_copy):
    ...
    for i in range(4):
        ...
        for j in range(4):
            ...
            for k in range(4):
                if list_matrix[k] == 14:
                    string_1 = GF(list_num[k])
                    string_1_int = bin_to_dec(string_1)
                    string_2 = GF(string_1_int)
                    string_2_int = bin_to_dec(string_2)
                    string_3 = GF(string_2_int)
...
def get_4_double(i_num, num, dir_key):
    ...
    for i in range(4):
        ...
        list[i] = SBox(line_number, row_number)
    list_new = []
    for i in range(4):
...
def AddRoundKey(clear_number, dir_key_extend):
    # 进行轮密钥加的操作
...
#加密算法
def AES_encrypt(clear_number, dir_key_extend):
    # 对明文进行轮密钥加
    dir_new_number = AddRoundKey(clear_number, dir_key_extend)
    ...
    for i in range(10):
        ...
        # 字节代换
        dir_1 = SubBytes(dir_new_number, 1)

        # 行移位
        dir_1 = ShiftRows(dir_1)

        # 定义列混合操作
        if i != 9:
            dir_1 = hex_to_int(dir_1)
            dir_1 = MixColumns(dir_1)

        # 定义轮密钥加
        dir_1 = AddRoundKey(dir_1, dir_key_extend_part)
        dir_new_number = dir_1

    return dir_new_number

#解密算法
def InvAES_encrypt(clear_number, dir_key_extend):
    # 对密文进行轮密钥加
    dir_key_extend_part = {
        0: dir_key_extend[40],
        1: dir_key_extend[41],
    ...
    dir_new_number = AddRoundKey(clear_number, dir_key_extend_part)
    ...
    for i in range(10):
        ...
        # 逆行移位
        dir_1 = InvShiftRows(dir_new_number)

        # 逆字节代换
        dir_1 = SubBytes(dir_1, 0)
        ...
        if i != 9:
            dir_1 = hex_to_int(dir_1)
            dir_1 = InvMixColumns(dir_1)
        dir_new_number = dir_1
    return dir_new_number


def print_zd(dir_num):
    # 测试输出字典
...
def reverse_ASCII(matrix):
    ...
    for i in range(len(matrix[0])):
        for j in range(4):
            hex_str = matrix[j][i]
            if hex_str:
                clear_number += chr(int(hex_str, 16))
    return clear_number
...
def assign_value():
    # 将输入16位的密钥转换成相对的字典格式
    miyao = entry.get()
    hex_strings = [f'0x{num}' for num in miyao]
    result = {}
    group_size = 4
    # 将16进制字符串列表按照每四个一组存储到字典中
    for i in range(0, len(hex_strings), group_size):
        key = i // group_size
        value = hex_strings[i:i + group_size]
        result[key] = value
        global keyy
        keyy = result
...
# 创建一个按钮，当按钮被点击时调用 assign_value 函数
button = tk.Button(my, text="提交", command=assign_value)
button.pack(pady=10)
...
print("key=",keyy)
# 得到扩展的密钥
Key_Expansion = KeyExpansion(keyy)
##测试结果：
####  文件植入
# 打开文件并读取内容
with open("clear", 'r', encoding='utf-8') as file:
    file_content = file.read()
    print("文件里的内容：",file_content)
    print("文件加密: ")
    clear_number = file_content
    clear_number = get_standard_input(clear_number)

    # 得到明文矩阵
    dir_clear_number = ASCII(clear_number)

    print("明文矩阵：")
    print_zd(dir_clear_number)  # 输出明文矩阵
    print("\n")

    print("扩展后的密钥：")
    print(Key_Expansion)  # 输出扩展密钥
    print("\n")

    dir_new_encrypt_number = AES_encrypt(dir_clear_number, Key_Expansion)

    print("密文: ",dir_new_encrypt_number)
    print("密文矩阵：")
    print_zd(dir_new_encrypt_number)  # 输出密文矩阵
    print("\n")

    dir_orinal_ = InvAES_encrypt(dir_new_encrypt_number, Key_Expansion)
    print_zd(dir_orinal_)  # 输出解密后的矩阵

    dir_ = get_outcome(dir_orinal_)
    print("解密后的明文:",dir_)  # 输出解密后的明文
